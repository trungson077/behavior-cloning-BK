...
for j in range(num_bins): #bat dau bang viec looping qua tung single bin
  ...
  for i in range(len(data['steering'])): #co lap goc lai cua bin hien tai co xu huong lap lai
    if data['steering'][i] >= bins[j] and data['steering'][i] <= bins[j +1]: # neu data o tren >= 20 va data<=200+1
      list_.append(i)  #list nay gom nhung steering angle cho 1 specific bin , bin ma hien tai dc lap di lap lai
...
def nvidia_model():
  model = Sequential()
  model.add(Convolution2D(24, 5, 5, subsample=(2,2), input_shape=(66,200,3),activation='elu')) #24 filters 5x5, stride =2
  model.add(Convolution2D(36, 5 ,5, subsample=(2,2), activation='elu'))

  model.add(Convolution2D(48, 5 ,5, subsample=(2,2), activation='elu')) #neu values of relu func <0 thi no se return ve 0, neu values >0 thi no se tra ve kq cua cua chinh no
